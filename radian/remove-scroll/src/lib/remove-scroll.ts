...
export class RadianRemoveScroll {
   ...
   * @default false
   */
  noIsolation = input(false, { transform: booleanAttribute });
  /**
   ...
   * @default false
   */
  inert = input(false, { transform: booleanAttribute });
  /**
   * allows pinch-zoom, however might work not perfectly for normal scroll
   */
  allowPinchZoom = input(false, { transform: booleanAttribute });
   ...
   */
  shards = input<Array<ElementRef<HTMLElement>>>();
  ...
  constructor() {
    ...
    const shouldPrevent = (event: WheelEvent | TouchEvent) => {
      ...
      if (!sourceEvent) {
        const shardNodes = (this.shards() || []).filter((node) =>
    ...
    effect(() => {
      if (this.inert()) {
        ...
        const allow = [elementRef, ...(this.shards() || [])].filter(Boolean);
