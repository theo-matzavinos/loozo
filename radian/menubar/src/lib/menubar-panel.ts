...
import { RadianMenuPanel } from '@loozo/radian/menu';
...
@Directive({
  selector: '[radianMenubarPanel]',
  providers: [
    {
      provide: RadianMenubarContentContext,
      useFactory: RadianMenubarPanel.contextFactory,
    },
  ],
  hostDirectives: [
    {
...
export class RadianMenubarPanel {
  /**
  ...
  private static contextFactory(): RadianMenubarContentContext {
    const context = inject(RadianMenubarContext);
    const menuPanel = inject(RadianMenuPanel);
    const hasInteractedOutside = signal(false);

    menuPanel.focusOutside.subscribe((event) => {
      const target = event.target as HTMLElement;
      const isMenubarTrigger = context
        .triggers()
        .some((item) => item.nativeElement.contains(target));
    ...
    menuPanel.interactionOutside.subscribe(() => {
