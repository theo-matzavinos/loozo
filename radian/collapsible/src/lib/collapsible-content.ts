...
@Directive({
  ...
  host: {
    'data-radian-collapsible-content': '',
    '[attr.id]': 'id()',
    '[attr.data-state]': 'context.state()',
    '[attr.data-disabled]': 'context.disabled() ? "" : null',
    '[attr.hidden]': '!context.open() || null',
    '[style]': `{
      '--radian-collapsible-content-height]': 'height()',
      '--radian-collapsible-content-width]': 'width()',
    }`,
  },
})
export class RadianCollapsibleContent {
  ...
  protected height = computed(() => `${this.dimensions().height}px`);
  protected width = computed(() => `${this.dimensions().width}px`);
