...
import { RadianPopperContentContext } from './popper-panel';
import { RadianPopperSide } from './popper';
...
@Directive({
  host: {
    '[style]': `{
      display: 'block',
      position: 'absolute',
      left: popperContentContext.arrowX(),
      top: popperContentContext.arrowY(),
      right: right(),
      bottom: bottom(),
      transformOrigin: transformOrigin(),
      transform: transform(),
      visibility: popperContentContext.shouldHideArrow() ? 'hidden' : '',
    }`,
  },
})
